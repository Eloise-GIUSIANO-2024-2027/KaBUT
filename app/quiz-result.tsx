...
export default function QuizPage() {
  ...
  const params = useLocalSearchParams<{ result?: string; quizId?: string }>();
          ...
          <Pressable
            onPress={() =>
              router.replace({
                ...
                params: { quizId: params.quizId ?? '', refresh: Date.now().toString() },
              })
            }
            style={[
              styles.button,
...
const styles = StyleSheet.create({
  ...
  card: { width: '100%', maxWidth: 560, borderRadius: UiTokens.radiusDefault, padding: 24, alignItems: 'center', gap: 16 },
  pie: { width: 200, height: 200, borderRadius: 999, justifyContent: 'center', alignItems: 'center' },
  pieCenter: { width: 130, height: 130, borderRadius: 999, justifyContent: 'center', alignItems: 'center' },
  scoreText: { fontSize: 34, lineHeight: 42 },
  helperText: { textAlign: 'center', fontSize: 16, lineHeight: 24, maxWidth: 460 },
  actions: { flexDirection: 'row', gap: 12, marginTop: 8 },
  button: { minWidth: 140, borderRadius: UiTokens.radiusButton, paddingVertical: 12, paddingHorizontal: 18, alignItems: 'center' },
  buttonText: { color: '#FFFFFF', fontSize: 16 },
  webButtonTransition: { transitionProperty: 'transform, background-color', transitionDuration: '0.2s' },
});